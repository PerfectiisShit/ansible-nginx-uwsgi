...
while True:
    ...
    if 'vassals' in dd:
        ...
        blacklisted_valssals = []
        if dd.get('blacklist'):
            blacklisted_valssals = [v['id'] for v in dd['blacklist']]

        if dd['vassals']:
            vassal_spaces = max([len(v['id']) for v in dd['vassals']])
            screen.addstr(2, 0, " VASSAL%s\tPID  \tLast Ready         " % (' ' * (vassal_spaces-6)), curses.A_REVERSE)
            for vassal in dd['vassals']:
                if vassal['accepting'] == 1:
                    if vassal['id'] in blacklisted_valssals:
                        blacklisted_valssals.remove(vassal['id'])
                    screen.addstr(row_pos, 0, " %s\t%d\t%s" %
                                  (vassal['id'].ljust(vassal_spaces),
                                   vassal['pid'],
                                   datetime.fromtimestamp(vassal['last_ready'])))
                    row_pos += 1

        if blacklisted_valssals:
            row_pos += 1
            vassal_spaces = max([len(v) for v in blacklisted_valssals])
            vassal_spaces = vassal_spaces if vassal_spaces >= 15 else 15
            screen.addstr(row_pos, 0, " VASSAL(offline)%s\tFirst Failure%s\tLast Attempt%s" %
                          (' ' * (vassal_spaces - 15), ' ' * 6, ' ' * 7), curses.A_REVERSE)
            row_pos += 1
            for bvassal in dd['blacklist']:
                if bvassal['id'] in blacklisted_valssals:
                    screen.addstr(row_pos, 0, " %s\t%s\t%s" %
                                  (bvassal['id'].ljust(vassal_spaces),
                                   datetime.fromtimestamp(bvassal['first_attempt']).isoformat(' '),
                                   datetime.fromtimestamp(bvassal['last_attempt']).isoformat(' ')))
                    row_pos += 1
